...
import { Search, Plus, Minus, Loader2, ShoppingBag, X, CreditCard, Banknote, ChevronUp } from 'lucide-react'
...
export default function SalesPage() {
    ...
    // The cart panel content — shared between desktop sidebar and mobile bottom sheet
    const CartContent = () => (
        <>
    ...
    return (
        <div className="animate-in fade-in duration-700">
            {/* ── DESKTOP LAYOUT ── */}
            <div className="hidden md:flex h-[calc(100vh-160px)] gap-8">
                {/* Products Grid */}
                        ...
                        {filteredProducts.map(p => (
                            ...
                            >
                                ...
                                <div className="mt-6 flex items-center justify-between border-t border-[#232327] pt-4">
                                    <div>
                                        <span className="text-xs font-bold text-[#22C55E]">${Number(p.sell_price).toLocaleString()}</span>
            ...
            <div className="flex flex-col md:hidden space-y-4">
                <header>
                    <h1 className="serif-title text-3xl font-light text-[#F3F3F3]">Punto de Venta</h1>
                    <div className="mt-4 relative">
                        <Search className="absolute left-0 top-1/2 -translate-y-1/2 h-4 w-4 text-[#A0A0A8]" />
                        <input
                            type="text"
                            placeholder="Buscar producto..."
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                            className="w-full bg-transparent border-b border-[#232327] py-3 pl-8 text-[#F3F3F3] placeholder:text-[#A0A0A8] focus:border-[#22C55E] focus:outline-none transition-colors text-sm"
                        />
                    </div>
                </header>
                ...
                <div className="grid grid-cols-2 gap-3 pb-32">
                    {filteredProducts.map(p => (
                        <button
                            key={p.id}
                            onClick={() => { addToCart(p); if (p.current_stock > 0) setCartOpen(true) }}
                            disabled={p.current_stock <= 0}
                            className={cn(
                                "bg-[#141418] border border-[#232327] rounded-xl p-4 text-left group transition-all",
                                p.current_stock <= 0 ? "opacity-40 grayscale cursor-not-allowed" : "hover:border-[#22C55E]/40 cursor-pointer active:scale-95"
                            )}
                        >
                            <p className="text-[9px] uppercase tracking-[0.2em] text-[#A0A0A8] mb-1">{p.brand}</p>
                            <h3 className="font-serif text-sm font-light text-[#F3F3F3] leading-tight">{p.name}</h3>
                            <div className="mt-3 flex items-center justify-between border-t border-[#232327] pt-3">
                                <div>
                                    <span className="text-xs font-bold text-[#22C55E]">${Number(p.sell_price).toLocaleString()}</span>
                                    <p className="text-[9px] text-[#A0A0A8] mt-0.5">{p.current_stock} uds</p>
                                </div>
                                <Plus className="h-4 w-4 text-[#A0A0A8] group-hover:text-[#22C55E] transition-colors" />
                            </div>
                        </button>
            ...
            {cart.length > 0 && (
                <button
                    ...
                    className="fixed bottom-20 left-1/2 -translate-x-1/2 md:hidden z-30 flex items-center gap-3 px-6 py-3 rounded-full bg-[#22C55E] text-black font-bold text-sm shadow-2xl shadow-[#22C55E]/30 active:scale-95 transition-transform"
                >
            ...
            {cartOpen && (
                <div className="fixed inset-0 z-50 md:hidden flex flex-col justify-end">
                    ...
                    {/* Sheet */}
                    <div className="relative bg-[#141418] border-t border-[#232327] rounded-t-3xl max-h-[85vh] flex flex-col animate-in slide-in-from-bottom duration-300">
                        {/* Handle */}
                        <div className="flex items-center justify-between px-6 py-4 border-b border-[#232327]">
                            <h2 className="serif-title text-xl font-light text-[#F3F3F3]">Bolsa de Venta</h2>
                            <button onClick={() => setCartOpen(false)} className="text-[#A0A0A8] hover:text-[#F3F3F3] transition-colors">
                                <X className="h-5 w-5" />
                            </button>
                        </div>
